from tkinter import *
import sqlite3
# from admin import view, insert, search, delete, update
...
def insert(title, author, year, isbn):
    ...
    try:
        ...
        msg_lbl.configure(text="Record with ISBN no {0} Exists...".format(isbn))
    conn.commit()
    conn.close()
...
def delete(id):
    ...
    msg_lbl.configure(text="Record with id of {} Deleted Successfully".format(id))
    conn.commit()
    conn.close()
...
def update(id, title, author, year, isbn):
    ...
    msg_lbl.configure(text="Record with isbn no {0} Updated Successfully".format(isbn))
    conn.commit()
    conn.close()
...
def get_selected_row(event):
    ...
    try:
        ...
        msg_lbl.configure(text="No Record Selected")
...
def search_command():
    ...
    if (len(title_text.get())==0 and len(author_text.get())==0 and len( year_text.get()) ==0 and len(isbn_text.get())==0):
        ...
        msg_lbl.configure(text="Fill at least One Entry")
    else:
        list_area.delete(0, END)
...
def add_command():
    if (len(title_text.get())==0 or len(author_text.get())==0 or len( year_text.get()) ==0 or len(isbn_text.get())==0):
        ...
        # showinfo(title='Information',message = msg)
        msg_lbl.configure(text="Fill Entries Required")
    else:
